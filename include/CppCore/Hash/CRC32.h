...
namespace CppCore
{
   ...
   protected:
      INLINE CRC32b(const uint32_t init = INIT) 
      {
         reset(init);
      }
   public:
   ...
   /// CRC32 Generic
   /// </summary>
   class CRC32g : public CRC32b<CRC32g>
   {
   public:
      CPPCORE_ALIGN64 static constexpr const uint32_t TABLE[] = 
      {
   ...
   public:
      INLINE CRC32g(const uint32_t init = INIT) : CRC32b(init) { }
      INLINE void reset(const uint32_t init = INIT)
      {
         CRC32b::reset(init);
      }
   ...
   /// <summary>
   /// CRC32 using Optimized ARM Intrinsics
   /// </summary>
   class CRC32s : public CRC32b<CRC32s>
   {
   public:
      using Hash::step;
      using Hash::hash;
      ...
      using CRC32b::XOROUT;
   public:
      INLINE CRC32s(const uint32_t init = INIT) : CRC32b(init) { }
      INLINE void reset(const uint32_t init = INIT)
      {
         CRC32b::reset(init);
      }
      ...
      INLINE void step(const void* data, size_t len)
      {
         ...
         if (mem + 4U <= end)
         {
            t32 = __crc32w(t32, *(uint32_t*)mem);
            mem += 4U;
         }
         if (mem + 2U <= end)
         {
            t32 = __crc32h(t32, *(uint16_t*)mem);
            mem += 2U;
         }
         if (mem < end)
            t32 = __crc32b(t32, *mem);
         mState = t32;
      }
   };
   using CRC32 = CRC32s;
   ...
   /// <summary>
   /// CRC32C using Optimized SSE42 Intrinsics
   /// </summary>
   ...
   public:
      ...
      INLINE void step(const void* data, size_t len)
      {
         ...
         if (mem + 2U <= end)
         {
            t32 = _mm_crc32_u16(t32, *(uint16_t*)mem);
            mem += 2U;
         }
         if (mem < end)
      ...
      INLINE void step64(const void* data, size_t len)
      {
         assert(len % 8 == 0);
      #if defined(CPPCORE_CPU_64BIT)
         uint64_t* mem = (uint64_t*)data;
         uint64_t  t64 = mState;
         while (len -= 8U)
            t64 = _mm_crc32_u64(t64, *mem++);
         mState = (uint32_t)t64;
      #else
         uint32_t* mem = (uint32_t*)data;
         uint32_t  t32 = mState;
   ...
   using CRC32C = CRC32Cs;
#elif defined(CPPCORE_CPUFEAT_ARM_CRC32)
   /// <summary>
   /// CRC32C using Optimized ARM Intrinsics
   /// </summary>
   class CRC32Cs : public CRC32b<CRC32Cs>
   {
   public:
   ...
   public:
      ...
      INLINE void step(const void* data, size_t len)
      {
         ...
         if (mem + 4U <= end)
         {
            t32 = __crc32cw(t32, *(uint32_t*)mem);
            mem += 4U;
         }
         if (mem + 2U <= end)
         {
            t32 = __crc32ch(t32, *(uint16_t*)mem);
            mem += 2U;
         }
         if (mem < end)
            t32 = __crc32cb(t32, *mem);
         mState = t32;
      }
      INLINE void step32(const void* data, size_t len)
      {
