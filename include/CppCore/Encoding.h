...
namespace CppCore
{
   ...
   public:
      ...
      public:
         ...
         {
            CPPCORE_ALIGN64 static constexpr uint16_t table_uppercase[] = 
            {
            ...
            CPPCORE_ALIGN64 static constexpr uint16_t table_lowercase[] =
            {
               0x3030, 0x3130, 0x3230, 0x3330, 0x3430, 0x3530, 0x3630, 0x3730,
            ...
            return UPPERCASE ? table_uppercase[byte] : table_lowercase[byte];
         }
      };
      ...
      INLINE static void encode(const void* in, char* out, size_t len, bool reverse = false, bool writeterm = false, bool uppercase = true)
      {
         ...
         if (reverse)
         {
            ...
            if (uppercase)
               while (len--)
                  *out16++ = Util::bytetohexint16<true>(*--in8);
            else
               while (len--)
                  *out16++ = Util::bytetohexint16<false>(*--in8);
         }
         else
         {
